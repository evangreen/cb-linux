...
wget_if_needed kernel.flags https://raw.githubusercontent.com/MilkyDeveloper/cb-linux/main/kernel/kernel.flags -O kernel.flags -q --show-progress
...
[[ -n "$VERSION" ]] || {
    wget_if_needed bzImage https://github.com/MilkyDeveloper/cb-linux/releases/latest/download/bzImage -O bzImage -q --show-progress
    wget_if_needed modules.tar.xz https://github.com/MilkyDeveloper/cb-linux/releases/latest/download/modules.tar.xz -O modules.tar.xz -q --show-progress
}
[[ -n "$VERSION" ]] && {
